...
void Player::Init(int x, int y, int vMoveSize, float jumpsize, float jumpTarget, int status, TSprite* sprite[], int arrsize, TSprite* sprGage)
{
	...
	//gage 세팅
	m_gage = new Gage();
	m_gage->Init(92,25, &m_Hp, sprGage,1);
	...
	for (int i = 0; i < arrsize; i++)
	{
		...
		else 
		{
			...
			Collision->m_BoundingVolume.right = (sprite[i]->m_Width / sprite[i]->m_AniInfo.m_FrmMax) - 100;
			Collision->m_BoundingVolume.bottom = sprite[i]->m_Height;
			m_PlayerAABB.push_back(Collision);
...
void Player::Update(float dtime)
{
	...
	if (m_CurrentStatus == MOVERIGHT || m_CurrentStatus == MOVELEFT)
	{
		...
		{
			SoundManager::GetInstance()->PlayOnce(SoundList::FIREMISS);
...
void Player::Draw(float dtime)
{
	...
	//애니메이션
	if (m_CurrentStatus == MOVELEFT || m_CurrentStatus == MOVERIGHT)
	{
		TSEngine::GetEngine()->DrawAnimation(backbuffer, m_Pos.x, m_Pos.y, m_SpriteAni[m_CurrentStatus], dtime * 5.0f);
	...
	//debug
	//TSEngine::GetEngine()->DrawTextT(TSEngine::GetEngine()->GetBackBufferDC(), 1, 50, RGB(255, 0, 255), "Player1 hp : %d", m_Hp);
...
void Player::AutoDown(int downSize, float dtime)
{
	GameObject::AutoDown(downSize, dtime);

}

int Player::CheckOutOfBoundary(int locationY)
{
...
void Player::KeyInput()
{
	...
	if (KeyState::InputKeyDown(0x57)) //w
	{
	...
	if (KeyState::InputKeyDown(0x47)) //gg
	{
			...
			)
		{
			...
			{
				SoundManager::GetInstance()->PlayOnce(SoundList::FIREEAT);
